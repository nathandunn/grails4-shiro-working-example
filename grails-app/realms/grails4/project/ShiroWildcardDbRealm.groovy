...
class ShiroWildcardDbRealm implements GrailsShiroRealm, SimplifiedRealm {
    ...
    ShiroWildcardDbRealm() {
        println "setting wildcard realm"
    ...
    AuthenticationInfo authenticate(AuthenticationToken authToken) {
        if (authToken instanceof UsernamePasswordToken) {
            ...
            ShiroUser user = ShiroUser.findByUsername(username)
            if (!user) {
            ...
            println "accountt: ${account}"
            if (!credentialMatcher.doCredentialsMatch(authToken, account)) {
                println "Invalid password (ShiroWildcardDbRealm)"
                throw new IncorrectCredentialsException("Invalid password for user ${username}")
            }
            println "returning  ${account}"
            return account
        }
...
class ShiroWildcardDbPrincipalHolder implements Serializable, PrincipalHolder {
    ...
    ShiroWildcardDbPrincipalHolder(ShiroUser user) {
        id = user.id
        userName = user.username

        println "principal holder"
    }

    Set<String> getRoles() {
        ShiroUser.get(id)?.roles?.collect { it.name } ?: []
    }
